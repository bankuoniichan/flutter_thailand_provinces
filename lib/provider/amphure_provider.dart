...
class AmphureProvider {
  ...
  static Future<List<AmphureDao>> all({int provinceId = 0, int? limit}) async {
    String? where;
    ...
    List<Map<String, dynamic>> mapResult = await ThailandProvincesDatabase.db!
        .query(TABLE_AMPHURES, where: where, whereArgs: whereArgs, limit: limit);
  ...
  static Future<List<AmphureDao>> search({String keyword = "", int? limit}) async {
    List<Map<String, dynamic>> mapResult = await ThailandProvincesDatabase.db!
        .query(TABLE_AMPHURES,
